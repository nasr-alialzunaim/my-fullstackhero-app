...
```bash
dotnet run --project src/Host/DNationalSystem.AppHost
```
...
```bash
dotnet run --project src/Host/DNationalSystem.Api      # needs external Postgres + Redis
```
...
src/
  ...
  Host/
    DNationalSystem.Api/                    API composition root
    DNationalSystem.AppHost/                .NET Aspire orchestrator
    DNationalSystem.DbMigrator/             One-shot migrate / seed runner
    DNationalSystem.Migrations.PostgreSQL/  EF Core migrations
...
- [ ] **Mail** — configure SMTP / SendGrid under `MailOptions` in
      `src/Host/DNationalSystem.Api/appsettings.json`.
...
```bash
dotnet test src/DNationalSystem.slnx       # integration tests require Docker
```
