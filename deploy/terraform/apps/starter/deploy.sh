#!/usr/bin/env bash
#
# One-command deploy of the DNationalSystem Starter Kit to AWS.
#
...
if [[ "$BUILD_API" == true ]]; then
  ...
  dotnet publish "$REPO_ROOT/src/Host/DNationalSystem.Api/DNationalSystem.Api.csproj" \
  ...
  dotnet publish "$REPO_ROOT/src/Host/DNationalSystem.DbMigrator/DNationalSystem.DbMigrator.csproj" \
    -c Release -r linux-x64 \
    /t:PublishContainer \
    -p:ContainerRegistry="$REGISTRY_HOST" \
