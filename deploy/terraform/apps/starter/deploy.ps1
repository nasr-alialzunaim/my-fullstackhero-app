...
if ($BuildApi) {
  ...
  dotnet publish "$RepoRoot/src/Host/DNationalSystem.Api/DNationalSystem.Api.csproj" `
  ...
  dotnet publish "$RepoRoot/src/Host/DNationalSystem.DbMigrator/DNationalSystem.DbMigrator.csproj" `
    -c Release -r linux-x64 `
    /t:PublishContainer `
    -p:ContainerRegistry="$registryHost" `
