...
dotnet run --project src/Host/DNationalSystem.DbMigrator -- apply

# Apply only to one tenant.
dotnet run --project src/Host/DNationalSystem.DbMigrator -- apply --tenant root

# Apply only the tenant catalog (no per-tenant pass).
dotnet run --project src/Host/DNationalSystem.DbMigrator -- apply --catalog-only
...
dotnet run --project src/Host/DNationalSystem.DbMigrator -- seed
...
DOTNET_ENVIRONMENT=Development \
  dotnet run --project src/Host/DNationalSystem.DbMigrator -- seed-demo
```
...
as `DNationalSystem.Api` (both files are linked into the project so they
...
| `DatabaseOptions__MigrationsAssembly`     | `DNationalSystem.Migrations.PostgreSQL`         |
...
```bash
dotnet publish src/Host/DNationalSystem.DbMigrator -c Release \
        ...
        - name: migrator
          ...
          env:
            ...
            - name: DatabaseOptions__MigrationsAssembly
              value: DNationalSystem.Migrations.PostgreSQL
```
...
- **Aspire**: `dotnet run --project src/Host/DNationalSystem.AppHost` —
...
- **Raw**: run the migrator once after pulling, before starting the
  ...
  # Then start the API
  dotnet run --project src/Host/DNationalSystem.Api
  ```
