...
public sealed class ForensicCaseConfiguration : IEntityTypeConfiguration<ForensicCase>
{
    public void Configure(EntityTypeBuilder<ForensicCase> builder)
    {
        ...
        builder.Property(x => x.CaseType).HasMaxLength(32);
        builder.Property(x => x.Status).HasConversion<string>().HasMaxLength(32);
        builder.Property(x => x.Priority).HasMaxLength(32);
        builder.Property(x => x.JurisdictionCode).HasMaxLength(64);
...
public sealed class CaseAssignmentConfiguration : IEntityTypeConfiguration<CaseAssignment>
{
    public void Configure(EntityTypeBuilder<CaseAssignment> builder)
    {
        ...
        builder.HasKey(x => x.Id);
        builder.Property(x => x.Id).ValueGeneratedNever();
        builder.Property(x => x.AssignmentRole).HasMaxLength(64);
        builder.HasIndex(x => new { x.CaseId, x.UserId });
        builder.HasIndex(x => x.UserId);
        builder.HasOne<ForensicCase>()
            .WithMany()
...
public sealed class CaseStatusHistoryConfiguration : IEntityTypeConfiguration<CaseStatusHistory>
{
    public void Configure(EntityTypeBuilder<CaseStatusHistory> builder)
    {
        ...
        builder.HasIndex(x => new { x.CaseId, x.ChangedAtUtc });
        builder.HasOne<ForensicCase>()
            .WithMany()
            .HasForeignKey(x => x.CaseId)
            .OnDelete(DeleteBehavior.Cascade);
