  ...
  return (
    <div className="space-y-8">
      <EntityPageHeader
        ...
        description={
          <>
            {t("health.description", { defaultValue: "Live process and dependency probes. Auto-refreshes every {{seconds}} seconds. Probe endpoints are unauthenticated so they can be scraped by load balancers and uptime monitors.", seconds: REFRESH_INTERVAL_MS / 1000 })}
          </>
        }
      >
      ...
      <StatStrip cols={4}>
        <Stat
          label={t("health.liveness", { defaultValue: "Liveness" })}
          value={<StatusGlyph status={liveStatus} />}
          hint={t("health.apiResponding", { defaultValue: "API process responding" })}
          tone={statusToTone(liveStatus)}
        />
        <Stat
          label={t("health.readiness", { defaultValue: "Readiness" })}
          value={<StatusGlyph status={readyStatus} />}
          hint={t("health.dependenciesReachable", { defaultValue: "Dependencies reachable" })}
          tone={statusToTone(readyStatus)}
        />
        <Stat
          label={t("health.checksHealthy", { defaultValue: "Checks healthy" })}
          value={isLoading ? "—" : checksHealthy.toString()}
          hint={t("health.registered", { defaultValue: "of {{count}} registered", count: readyEntries.length || "—" })}
          tone={checksHealthy > 0 ? "success" : "default"}
        />
        <Stat
          label={t("health.checksFailing", { defaultValue: "Checks failing" })}
          value={isLoading ? "—" : (checksFailing + checksDegraded).toString()}
          hint={
            checksDegraded > 0
              ? t("health.degradedUnhealthy", { defaultValue: "{{degraded}} degraded · {{unhealthy}} unhealthy", degraded: checksDegraded, unhealthy: checksFailing })
              : t("health.unhealthy", { defaultValue: "{{count}} unhealthy", count: checksFailing })
          }
      ...
      {live.isError && (
        <ErrorBand
          kind="liveness"
          message={live.error instanceof Error ? live.error.message : t("health.livenessFailed", { defaultValue: "Liveness probe failed." })}
        />
      )}
      {ready.isError && (
        <ErrorBand
          kind="readiness"
          message={ready.error instanceof Error ? ready.error.message : t("health.readinessFailed", { defaultValue: "Readiness probe failed." })}
        />
      )}

      <ProbeSection
        title={t("health.liveness", { defaultValue: "Liveness" })}
        path="/health/live"
        result={live.data}
        loading={live.isLoading}
        description={t("health.livenessDescription", { defaultValue: "Process is up and serving requests. No external dependencies are checked." })}
      />

      <ProbeSection
        title={t("health.readiness", { defaultValue: "Readiness" })}
        path="/health/ready"
        result={ready.data}
        loading={ready.isLoading}
        description={t("health.readinessDescription", { defaultValue: "All registered dependency checks must report Healthy for the probe to return 200. A 503 here flips the API out of the load-balancer pool." })}
      />
    </div>
  );
}
...
function ProbeSection({
  ...
  return (
    ...
    >
      {loading ? (
        <div className="py-6 text-sm text-[var(--color-muted-foreground)]">{t("health.probing", { defaultValue: "Probing…" })}</div>
      ) : !result || result.results.length === 0 ? (
        <div className="flex items-center gap-3 py-5">
          <Activity className="h-4 w-4 text-[var(--color-muted-foreground)]" />
          <div>
            <div className="text-sm font-medium">{t("health.noChecks", { defaultValue: "No dependency checks reported." })}</div>
            <div className="text-xs text-[var(--color-muted-foreground)]">
              {t("health.probeResponded", { defaultValue: "The probe responded with status" })} <code className="code-chip">{result?.status ?? "—"}</code>.
            </div>
          </div>
        </div>
      ) : (
  ...
  if (status === "Healthy") {
    return (
      <span className="inline-flex items-center gap-2">
        ...
        <span>{t("health.healthy", { defaultValue: "Healthy" })}</span>
      </span>
    );
  }
