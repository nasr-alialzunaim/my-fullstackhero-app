# DNationalSystem .NET Starter Kit
...
| `src/Host/DNationalSystem.Api` | Composition-root Web API host. |
| `src/Host/DNationalSystem.AppHost` | .NET Aspire orchestrator (Postgres, Redis, MinIO, migrator, API, **both React apps**). |
| `src/Host/DNationalSystem.DbMigrator` | One-shot migrate/seed runner. DB is **not** migrated at API startup. |
| `src/Host/DNationalSystem.Migrations.PostgreSQL` | All EF migrations, organized per-module by folder. |
...
dotnet run --project src/Host/DNationalSystem.AppHost   # one-time: npm install in clients/admin & clients/dashboard

dotnet build src/DNationalSystem.slnx                   # build backend
dotnet run --project src/Host/DNationalSystem.Api       # API only → https://localhost:7030 (/scalar)
dotnet test src/DNationalSystem.slnx                    # tests — integration tests REQUIRE Docker
...
```bash
dotnet run --project src/Host/DNationalSystem.DbMigrator -- apply [--seed]
dotnet run --project src/Host/DNationalSystem.DbMigrator -- list-pending
```
